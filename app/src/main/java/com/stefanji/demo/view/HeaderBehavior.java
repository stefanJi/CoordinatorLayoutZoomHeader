...
package com.stefanji.demo.view;
...
abstract class HeaderBehavior<V extends View> extends ViewOffsetBehavior<V> {
    ...
    private ValueAnimator revertOverScroll;

    private void startRevert() {
        if (revertOverScroll != null && revertOverScroll.isRunning()) {
            revertOverScroll.cancel();
        }
        revertOverScroll = ValueAnimator.ofInt(getTopAndBottomOffset(), 0)
                .setDuration(300);
        revertOverScroll.setInterpolator(new DecelerateInterpolator());
        revertOverScroll.addUpdateListener(animation -> {
            int offset = (int) animation.getAnimatedValue();
            Log.d("JY", "update offset: " + offset);
            setTopAndBottomOffset(offset);
        });
        revertOverScroll.start();
    }
    ...
    public boolean onTouchEvent(
        ...
        switch (ev.getActionMasked()) {
            case MotionEvent.ACTION_MOVE:
                ...
                scroll(parent, child, dy, getMaxDragOffset(child), 900 /*TODO(jy) 可以下拉的最大幅度*/);
                break;
    ...
    int setHeaderTopBottomOffset(CoordinatorLayout parent, V header, int newOffset, int minOffset, int maxOffset) {
        ...
        Log.d("JY", "newOffset:" + newOffset + " minOffset:" + minOffset + " maxOffset:" + maxOffset);
        if (minOffset != 0 && curOffset >= minOffset && curOffset <= maxOffset) {
            ...
            if (curOffset != newOffset) {
                Log.d("JY", "inner set newOffset:" + newOffset);
    ...
    final boolean fling(
        ...
        final int cur = getTopAndBottomOffset();
        Log.d("JY", "fling velocityY:" + velocityY + " curOffset:" + cur + " minOffset:" + minOffset + " maxOffset:" + maxOffset);
        scroller.fling(
                0,
                cur, // curr
                0,
    ...
    private class FlingRunnable implements Runnable {
        ...
        public void run() {
            if (layout != null && scroller != null) {
                if (scroller.computeScrollOffset()) {
                    final int curY = scroller.getCurrY();
//                    Log.d("JY", "fling scroller run:" + curY);
                    setHeaderTopBottomOffset(parent, layout, curY);
